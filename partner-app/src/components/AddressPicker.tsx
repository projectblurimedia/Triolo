 ...
 * Pincode as a 6-digit field. Used by `WorkerRegistrationScreen`/`BusinessRegistrationScreen`
 * (editing an existing profile, post-auth) and `RegisterWorkerScreen`/`RegisterBusinessScreen`
 * (creating one, pre-auth) — this app has no separate account-level location field anywhere;
 * registration only ever asks for Full Name/Mobile Number (see `RegisterWorkerScreen`'s own
 * doc comment) plus whichever capability's own address. "Use my current location" reverse-geocodes via
 ...
 * afterward does NOT clear the detected `latitude`/`longitude`, since correcting one field
 * doesn't invalidate an otherwise-correct GPS fix.
 */
