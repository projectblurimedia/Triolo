...
**Home menu**: the header's menu icon opens `HomeMenuModal`, a right-side drawer (matching a reference drawer design) that slides in/out via its own `Animated.timing` on `translateX` (not the platform `Modal`'s built-in `animationType`, which has no directional-slide option) with two entry points — "Become a Worker" and "Become a Business", each opening a real full-screen form (`WorkerProfileModal`/`BusinessProfileModal`: category chips, `LocationPicker`, `ImagePickerField` for up to 6 photos, submit) that calls `POST /workers/me/profile` / `POST /businesses/me/profile` via multipart `FormData` (`apiClient.postForm`). The drawer's header height is computed the same way `GradientHeader`'s is (`insets.top + 14`), so it always lines up with the app's own top header regardless of device. Its slide uses `timing`, not `spring`, deliberately — see `.cloud/architecture.md`'s "Header" section for why a spring's unpredictable settle time caused a real bug here. The drawer also has an "App Settings" section (Theme + Logout, reusing the same shared modals Profile's menu uses). `HomeHeader` (holding the drawer's visibility state) wraps `GradientHeader` the same way `ProfileHeader` does for its own menu.
