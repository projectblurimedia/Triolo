...
React Native (Expo) + TypeScript client — the consumer-facing app. Feature-based structure — see `../.cloud/architecture.md`. For the Worker/Business-facing app, see `../partner-app/README.md`.
...
src/
  ...
  components/     Button, TextField, ScreenContainer, LanguageSwitcher, ThemePickerModal, LogoutConfirmModal,
                  LogoutOverlay, LoadingIndicator, LocationPicker, ImagePickerField, WorkerProfileModal,
                  BusinessProfileModal, GradientHeader, ProfileHeader, ProfileMenuModal, HomeHeader, HomeMenuModal
  navigation/     RootNavigator (language + theme hydration gate → auth vs. app), AuthNavigator (stack),
                  AppNavigator (bottom tabs)
  services/       apiClient (fetch wrapper, auto access-token refresh on 401, plus postForm for multipart),
                  authService, workersService, businessesService
  hooks/          React Query mutations wrapping authService/workersService/businessesService
  state/          Zustand stores: authStore (session + isLoggingOut, persisted minus isLoggingOut),
                  settingsStore (language, persisted), themeStore (theme mode: system/light/dark, persisted)
...
Full registration/login flow against the backend Auth module: name/mobile/email/location → OTP request → OTP verify → session persisted (AsyncStorage). Access-token refresh on 401 is wired in `apiClient`. Every self-registered account is a plain `user` — there's no role picker at registration (see `.cloud/architecture.md`'s "Account Model"); Worker/Business are optional capabilities added afterward via Home's menu.

**Welcome screen**: a gradient hero panel (brand icon, app name, tagline) followed by a single "Create Account" `Button` and a plain "Already have an account? Log In" text link — no role picker. Doesn't use the shared `ScreenContainer` (its fixed padding would clip the edge-to-edge hero bleed) — composes its own `SafeAreaView`s instead. **Logout** shows a full-screen `LogoutOverlay` (brand gradient, `LoadingIndicator` dots orbiting the app's `shapes` mark) for the duration of the request, driven by `authStore.isLoggingOut` (set in `useLogout()`'s `onMutate`/`onSettled`, excluded from persistence) and rendered from `App.tsx` above whichever navigator is mounted — replacing what used to be an abrupt, feedback-free stack swap straight to Welcome.

**Registration fields**: full name, mobile number, email, and location (`LocationPicker` — "Use my current location" via `expo-location`, reverse-geocoded to a readable address, with a manual-text fallback; no map/pin-drop UI yet, so manual entry never produces coordinates, only an address string).
...
**Home menu**: the header's menu icon opens `HomeMenuModal`, a right-side drawer (matching a reference drawer design) that slides in/out via its own `Animated.timing` on `translateX` (not the platform `Modal`'s built-in `animationType`, which has no directional-slide option) with two entry points — "Are You a Worker?" and "Do You Have a Shop?", each opening a real full-screen form (`WorkerProfileModal`/`BusinessProfileModal`: left-side `chevron-left` back button, not a close "X" — these are forms, not dismissible overlays; category chips, `LocationPicker`, `ImagePickerField` for up to 6 photos, submit) that calls `POST /workers/me/profile` / `POST /businesses/me/profile` via multipart `FormData` (`apiClient.postForm`). The drawer's header height is computed the same way `GradientHeader`'s is (`insets.top + 14`), so it always lines up with the app's own top header regardless of device. Its slide uses `timing`, not `spring`, deliberately — see `.cloud/architecture.md`'s "Header" section for why a spring's unpredictable settle time caused a real bug here. The drawer also has an "App Settings" section (Language + Theme + Logout, reusing the same shared modals Profile's menu uses). `HomeHeader` (holding the drawer's visibility state) wraps `GradientHeader` the same way `ProfileHeader` does for its own menu.

**Image picking** (`ImagePickerField`): gallery-only (`expo-image-picker`, no camera), pick-then-upload-on-submit — picked images sit as local URIs in the form's state for preview; the actual multipart upload only happens when the surrounding form submits. **Location** (`LocationPicker`, shared by registration and both profile forms): same "detect or type manually" pattern throughout the app.
...
Worker/Business search, booking, shopping list/orders, notifications, admin (including reviewing `verification_status` on capability profiles) — Services and Bazaar tabs are placeholders, and Home's search bar/filters are UI-only, until those modules land. Home's notification header icon still has no destination yet (menu does, as of the Worker/Business profile forms). Only English and Telugu exist; the architecture (enum-based language type, per-key resource files) supports adding more without touching business logic, per `docs/localization.md`. No map/pin-drop location UI yet — `LocationPicker`'s manual fallback is address-text-only, no coordinates.
...
- Registration's new fields (email, location) smoke-tested end-to-end against a live local backend: health check, OTP request with the new payload shape, validation rejection for missing fields, and auth-gated Workers/Businesses routes all responded correctly.
- Not yet run on a real device/emulator — do that before considering the tab bar / gradient header / dark mode / fonts / location permission prompts / image picker production-verified (web export only proves the bundle builds, not real-device rendering, and permission-gated native APIs in particular need a real device).
