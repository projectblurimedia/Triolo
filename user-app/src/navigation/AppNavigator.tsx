...
import { PrepareListScreen } from '@/screens/lists/PrepareListScreen';
...
export function AppNavigator() {
  ...
  const titles: Record<keyof MainTabParamList, string> = {
    ...
    PrepareList: t('tabs.prepareList'),
  ...
  const subtitles: Partial<Record<keyof MainTabParamList, string>> = {
    ...
    PrepareList: t('prepareList.tagline'),
  ...
  return (
    ...
    >
      ...
      />
      {/* No local UI state needed yet (unlike Home/Services/Bazaar/Profile's own header
          wrappers), so this tab just uses the default screenOptions-provided GradientHeader
          — no options.header override. */}
      <Tab.Screen name="PrepareList" component={PrepareListScreen} />
      <Tab.Screen
        name="Bazaar"
