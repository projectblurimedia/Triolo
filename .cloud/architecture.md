...
**Skill/category chips are multi-select, not single-select** in both `WorkerProfileModal` and `BusinessProfileModal` — a worker can have several skills, a shop can span several categories. Each chip's active state is `array.includes(key)` toggled on press (add/remove from the array), not a single selected-key comparison. When the `"other"` chip is among the selected values, a conditional `TextField` appears (`otherSkillLabel`/`otherCategoryLabel`) for the free-text description — required by the backend's `.refine()` validation whenever `"other"` is selected, so the client mirrors that requirement before submitting. `BusinessProfileModal` additionally has a Yes/No delivery-availability chip pair; selecting "Yes" reveals a `deliveryPricePerKm` `TextField` (also backend-required in that case). All of these fields are JSON-stringified (arrays) or stringified booleans/numbers client-side before being appended to the `FormData`, since multipart fields are all strings — see `backend/src/common/utils/parseMultipartField.ts`'s `parseJsonIfString`/`parseBooleanIfString` on the receiving end.
