...
**Home's menu drawer** (`HomeMenuModal`) surfaces two entry points — "Register as a Worker" and "Register as a Business" — for an already-logged-in `user` account to express intent to become a Worker/Business. There is no backend support yet for upgrading an existing account's role (registration today only branches into User/Worker/Business at initial sign-up, per `.cloud/project-context.md`), so both currently just show a localized "coming soon" alert rather than navigating anywhere — this is intentionally a thin placeholder, not a partial implementation, matching the same pattern Services/Bazaar tabs already use. Wire these to a real in-app upgrade flow once that business flow is designed and the Worker/Business modules exist. It also carries an "App Settings" section (reusing the existing `ThemeSwitcher` and `useLogout`) so theme/logout are reachable from Home too, not just Profile's own menu.

**Both `HomeMenuModal` and `ProfileMenuModal` use the platform `Modal`'s built-in `animationType="fade"` plus `statusBarTranslucent`** — not a custom `Animated`-driven transition. `HomeMenuModal` originally drove its own slide-in/out with `Animated.spring`/`Animated.timing` and manual mount-tracking state to avoid cutting the close animation short; that entire mechanism was removed in favor of the native fade, since a hand-rolled animation lifecycle is more surface area for exactly this class of bug (a slow/interrupted custom animation left the native `Modal` "visible" — and therefore still swallowing all touches — longer than the drawer visually appeared open, which made reopening it right after closing intermittently feel unresponsive). Any new modal in this app should default to the platform's own `animationType`/`statusBarTranslucent` rather than reaching for a custom `Animated` transition unless there's a concrete visual requirement (e.g. a direction-specific slide) that the built-in types can't express.
