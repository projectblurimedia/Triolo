...
**Profile is the exception to the shared header**: it uses `ProfileHeader` instead of the generic per-tab `GradientHeader` config, because its menu icon needs local state (modal visibility) that a stateless header render function in `AppNavigator`'s `screenOptions` can't hold. `ProfileHeader` wraps `GradientHeader` plus `ProfileMenuModal` (language/theme/logout, previously inline in the screen body) as a self-contained unit — any future screen needing header-triggered local UI (a menu, a popover) should follow this same wrapper-component pattern rather than trying to lift state into the navigator config. Profile's body itself is role-gated: a static ratings/posts section shows only for `worker`/`business_owner`/`business_staff` accounts, not plain `user` accounts — this is placeholder content (no ratings/reviews or posts module exists yet) but the role check itself is real and should be reused, not re-implemented, when those modules land.
