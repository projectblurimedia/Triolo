...
**Home's menu drawer** (`HomeMenuModal`) surfaces two entry points — "Become a Worker" and "Become a Business" — opening `WorkerProfileModal`/`BusinessProfileModal` (full-screen forms: skill/shop category chips, experience or shop name, `LocationPicker`, `ImagePickerField`, submit) which add the respective capability to the current `user` account via `POST /workers/me/profile` / `POST /businesses/me/profile` — see the Account Model section below. It also carries an "App Settings" section so theme/logout are reachable from Home too, not just Profile's own menu.
...
Every self-registered account is a plain `user` — not one of an exclusive set of roles chosen at registration. **Worker and Business are optional capabilities** a `user` account can add, either during onboarding (Home menu → "Become a Worker"/"Become a Business") or, in principle, at any later point — not an exclusive choice made once at signup. Adding a capability creates a linked `worker_profiles`/`business_profiles` row (backend modules `workers`/`businesses`, each owning its own table per the module-boundary rule above) whose own `id` is that capability's distinct identifier (worker_id / business_id) and whose own `verification_status` (`pending_verification` → `verified`/`rejected`) gates admin trust for that capability specifically — independent of `accounts.status`, which is `active` immediately for every self-registered account regardless of which capabilities it later gains. `business_staff` (an employee added by a business owner) and `admin` are unrelated to this model — provisioned differently, never self-registered. This superseded an earlier design where `role` was chosen once at registration (`user`/`worker`/`business_owner`) and Worker/Business accounts sat in `pending_verification` from the moment the account itself was created; `WelcomeScreen`'s three role-selection cards were removed accordingly (see above).
