...
**`"other"` is a dedicated "+ Add New" chip, not one of the toggleable category options** — `SKILL_CATEGORIES`/`SHOP_CATEGORIES` no longer include an `"other"` entry at all; instead, an always-visible dashed-border `+ Add New` chip is appended after the fixed grid (and after any already-committed custom entries), rendered the same way regardless of state, so it's always available to add another. Tapping it opens an inline `TextField` + circular gradient "Done" button (`otherInputValue`/`showOtherInput` state). Confirming (the Done button or the keyboard's own `returnKeyType="done"`) pushes the typed text into a local `otherSkillEntries`/`otherCategoryEntries` array, clears the input, and closes it — tapping `+ Add New` again reopens the input for another entry. Each committed entry renders as its own small removable gradient chip (tapping one removes it by index, since it isn't a fixed key with array-membership toggling). At submit time, if any custom entries exist, `"other"` is appended to the fixed-category array being sent and all entries are joined with `", "` into the single `otherSkillDescription`/`otherCategoryDescription` string the backend's `.refine()` validation expects (the backend has no concept of *multiple* other-entries — the join happens client-side).
...
**The `+ Add New` chip is a filled tinted pill, not a dashed outline** — `borderColor: accentColor` + `backgroundColor: ${accentColor}16` (a light alpha tint, matching `LocationPicker`'s detect-button tint convention) plus a small solid-fill circular badge (`addNewBadge`, 16px, accent-colored background with a white `plus` glyph) instead of a bare icon — reads as a deliberately-designed action chip rather than a placeholder/disabled-looking dashed box. `commitOtherEntry` does a case-insensitive duplicate check (`entry.toLowerCase() === trimmed.toLowerCase()`) before appending — typing a name that already exists (regardless of case) doesn't create a second chip; it moves the existing entry to the front of the array instead, so "you already added this" reads as the chip visibly jumping to the front rather than silently no-op'ing.
