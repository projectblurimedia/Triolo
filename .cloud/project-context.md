...
- **Workers** (`backend/src/modules/workers/`): `POST /workers/me/profile` (multipart — **multiple** skill categories (`skillCategories[]`, a worker can select more than one), an `otherSkillDescription` free-text field required when `"other"` is among them, experience years, location, 0-6 portfolio photos uploaded to Cloudinary), `GET /workers/me/profile`. One profile per account (409 if one already exists). 6 unit tests passing (mocked repository + mocked Cloudinary upload).
- **Businesses** (`backend/src/modules/businesses/`): mirrors Workers exactly, shop-shaped — `POST /businesses/me/profile` (shop name, **multiple** shop categories (`shopCategories[]`), an `otherCategoryDescription` free-text field required when `"other"` is among them, location, 0-6 shop photos, `deliveryAvailable` boolean + `deliveryPricePerKm` required when true), `GET /businesses/me/profile`. 7 unit tests passing.
