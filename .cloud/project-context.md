...
## Planned Next: Registration/Profile Overhaul (not started)

Confirmed with the user, not yet implemented — full plan at `C:\Users\MANIKANTA\.claude\plans\sparkling-seeking-rose.md` (Phases 2-4). Summary: registration expands to capture `email` and **full GPS coordinates** (lat/lng, reverse-geocoded to a readable address) — a **confirmed, deliberate override of the "No live GPS in MVP" decision above**, which will need updating in that change — then branches post-OTP-verify into role-specific profile completion: Business (shop name/category, shop geolocation, shop images) or Worker (skill category, experience, location, portfolio images). Images upload via Cloudinary (backend-mediated multer→Cloudinary pipeline, modeled on a pattern proven in a sibling project, but using Triolo's own credentials via env vars — never shared/copied secrets). Needs 2 new DB tables (`worker_profiles`, `business_profiles`), 2 new backend modules (`workers`, `businesses`), new file-upload infra, and a multi-step mobile registration wizard (2 new native deps: `expo-location`, `expo-image-picker`). Scoped as its own dedicated work session per the project's Definition of Done (tests at each step, not just the happy path).

**Account-model change (confirmed, also not yet implemented)**: every self-registered account is fundamentally a `user` — not one of an exclusive set of roles picked once at signup. Worker and Business become **additive capabilities**: a person can optionally add Worker details (skills/experience/portfolio) and/or Business details (shop info/images) on top of their base User account — either during registration or later. Submitting those details flags the account accordingly and generates a distinct `worker_id`/`business_id`. This supersedes `account_role`'s current single-exclusive-value-at-registration model and is *why* `HomeMenuModal`'s "Become a Worker"/"Become a Business" entries exist as placeholders today — under this model they become real, wired-up upgrade flows rather than a dead end. `business_staff`/`admin` are unrelated to this pivot (assigned differently, not self-registered) and stay as-is.
